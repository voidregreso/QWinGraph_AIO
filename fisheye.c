...
/*--------------------------------------------------------------------*/
/*			Modified by AiO											  */
/*		Added function to jump to a node location					  */
/*--------------------------------------------------------------------*/
...
#endif
{
	double xdiff, ydiff, dist1, dist2, help;
	...
	dist2 = (double)fe_scaling * dist1 / help;
        ...
        if ( dist1 != 0.0 ) dist2 /= dist1;     /* Ilfak Guilfanov, 30.05.2001 */

	*resx = sfocus_x + (int)(xdiff*dist2);
	*resy = sfocus_y + (int)(ydiff*dist2);
}
...
*		Scale_set_location
*	ABS to rel
*		xpos = (X*G_stretch)/G_shrink - V_xmin;
*		ypos = (Y*G_stretch)/G_shrink - V_ymin;
*
*/
#ifdef ANSI_C
int set_node_focus(long x, long y)
#else
int set_node_focus(x, y)
	long x, y;
#endif
{
	long local_x, local_y;
	local_x =   (x * G_stretch)/G_shrink ;
	local_y =  (y * G_stretch)/G_shrink ;
	set_fe_xfocus( local_x);
	set_fe_yfocus( local_y);
	return 1;
}
