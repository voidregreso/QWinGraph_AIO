...
void MainWindow::printFile()
{
  ...
  if ( pr.exec() == QDialog::Accepted )
  {
    ...
    int pwidth = qp->pageRect(QPrinter::DevicePixel).width();
    int pheight = qp->pageRect(QPrinter::DevicePixel).height();
  ...
  if ( obj == canvas->viewport() )
  {
    ...
    else if ( event->type() == QEvent::Wheel )
    {
      ...
      else
      {
        ...
        QAbstractSlider* slider = (wevent->angleDelta().y() / wevent->angleDelta().x() > 0 ? canvas->verticalScrollBar() :
...
MainWindow::MainWindow() {
  ...
  for (int i = 0; i < argc; i++ ) {
    argv[i] = _strdup(args.at(i).toUtf8().data());
  }
  argv[argc] = NULL;
  vcg_main(argc, argv);
  display_complete_graph();
